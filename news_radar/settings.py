...
_DIGEST_TOP_N_MIGRATED = "migrated_digest_top_n_v2"
...
def get_settings(*, refresh: bool = False) -> dict[str, Any]:
    ...
    if stored and not load_setting(_DIGEST_TOP_N_MIGRATED, False):
        # Old default was Top3; digests now carry Top5 unless user chose otherwise.
        if int(out.get("morning_push_top_n") or 0) <= 3:
            out["morning_push_top_n"] = int(cfg.MORNING_PUSH_TOP_N)
            save_setting("runtime", {**stored, "morning_push_top_n": out["morning_push_top_n"]})
        save_setting(_DIGEST_TOP_N_MIGRATED, True)
